...
import { Plus, Trash2, BadgeCheck, CreditCard, Building2 } from 'lucide-react'
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-3xl border border-border bg-card p-6 md:p-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-display text-xl font-extrabold">Funding sources</h2>
            <p className="text-sm text-muted-foreground">
              Top up campaigns from GCash, Maya, or your bank (powered by Xendit).
            </p>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle>Add a funding source</DialogTitle>
              </DialogHeader>
        ...
        <div className="mt-5 space-y-3">
          {methods.length === 0 ? (
            <div className="rounded-2xl border-2 border-dashed border-border bg-muted/30 p-8 text-center">
              <p className="font-medium">No funding sources yet</p>
              <p className="mt-1 text-sm text-muted-foreground">
                Add a card, GCash, Maya, or bank to fund campaigns.
              </p>
            </div>
          ) : (
            methods.map((m) => {
              const Icon = m.type === 'bank' ? Building2 : CreditCard
              return (
                <div
                  key={m.id}
