...
todos:
  ...
  - id: doc-brand-journey
    content: "Lock brand flow: campaign fields draft save fund publish FB-TikTok one-mode goal-or-budget PHP10k net to go live default-included submissions reject to exclude realtime or pre-payout breakdown monthly batch with brand confirmation"
    status: pending
  ...
  - id: doc-money-movement
    content: "Document money rules: 15% platform fee default 20% performance (80% headline) TikTok yellow basket 50/50 line gross PHP10k net pool auto-pause monthly payout batch with brand confirmation rejection clawback failed payout retries brand refund available vs reserved caps vs snapshots when line finalized"
    status: pending
...
**What this folder is:** **Behavior and rules** for the MVP — what brands and creators do, how money moves, and what we ship in copy. **Sign-in and roles** live in [Auth & sign-in](02-auth-and-signin.md). This doc does not replace Terms or legal; we align legal text with what ships in the product.
...
| [02-auth-and-signin.md](02-auth-and-signin.md) | One email path, verification, Brand vs Creator role, route rules, flowchart |
...
| [04-brand-flow.md](04-brand-flow.md) | Brand: sign-in pointer, campaign fields, draft → fund → publish, inbox, monthly payout view |
